...
static func generate(root: Node, extra_bindings: Array = []) -> Error:
    ...
    if scene_path.is_empty():
        push_warning("[UICodeGen] generate(): scene_file_path empty (forgot to set it before calling?)")
        return ERR_INVALID_PARAMETER
    ...
    if FileAccess.file_exists(gd_path):
        ...
        user_code_content = _extract_region(existing, _REGION_USER_CODE_START, _REGION_USER_CODE_END)
        if user_var_content.is_empty() and user_code_content.is_empty() and existing.find(_REGION_AUTO_BIND_START) < 0:
            push_warning("[UICodeGen] File exists but has no region markers. Refusing to overwrite: %s" % gd_path)
            return ERR_ALREADY_EXISTS
    var code: = _generate_code(root, scene_path, user_var_content, user_code_content, extra_bindings)
...
static func _generate_code(
    ...
    for entry in extra_bindings:
        ...
        if bound_names.has(ev_name):
            ...
            push_warning("[UICodeGen] @outlet '%s' (var '_%s') 被丢弃 — 该变量名已被其它节点占用,请在 figma 重命名该节点或避免多个同名节点同时打 @outlet" % [ev_path, ev_name])
            continue
...
class UIBindContextMenu extends EditorContextMenuPlugin:
