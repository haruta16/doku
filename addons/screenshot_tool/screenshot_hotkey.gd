...
func _input(event: InputEvent) -> void :
    if not (event is InputEventKey) or not event.pressed or event.echo:
        return
    if event.keycode == KEY_T and event.shift_pressed and (event.meta_pressed or event.ctrl_pressed):
        _capture()
...
func _capture() -> void :
    ...
    var root_path: String = ProjectSettings.globalize_path("res://").get_base_dir().get_base_dir().path_join(SCREENSHOT_FOLDER)
    ...
    var filename: String = "Screenshot_%d%02d%02d_%02d%02d%02d_%03d.png" % [
