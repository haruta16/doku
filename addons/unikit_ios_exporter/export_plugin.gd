...
class IOSUniKitExportPlugin extends EditorExportPlugin:
    ...
    func _inject_info_plist() -> void :
        ...
        if not FileAccess.file_exists(json_path):
            push_error(("[UniKitIOSExporter] info_plist_patch.json 不存在: %s。该文件应跟随仓库分发,"
                + " 请检查 git 状态或联系 SDK 商业产品获取真品配置(详见 ios/build/README.md Step 3)。") % json_path)
            return
        ...
        if not FileAccess.file_exists(plist_path):
            push_error(("[UniKitIOSExporter] 目标 Info.plist 不存在: %s。请确认 Godot 导出已完成,"
                + " 且导出路径命名规则未变(预期 <export_dir>/<scheme>/<scheme>-Info.plist)。") % plist_path)
            return
        ...
        print("[UniKitIOSExporter] _inject_info_plist 完成: success=%d fail=%d skip=%d (target=%s)"
    ...
    func _strip_empty_unused_usage_descriptions(plist_path: String) -> void :
        ...
        for key: String in _UNUSED_USAGE_DESCRIPTION_KEYS:
            ...
            var extract_rc: int = OS.execute("plutil", 
                ["-extract", key, "raw", plist_path], extract_out, true)
            if extract_rc != 0:
            ...
            if value != "" and value != "<NULL>":
                ...
                continue
            var rm_out: Array = []
            var rm_rc: int = OS.execute("plutil", 
                ["-remove", key, plist_path], rm_out, true)
            if rm_rc == 0:
                print("    ✓ removed empty %s" % key)
            else:
                push_warning("[UniKitIOSExporter] plutil -remove %s 失败 rc=%d output=%s"
    ...
    func _patch_dummy_cpp_filetype() -> void :
        ...
        if content.contains(new_str):
            print("  dummy.cpp file type already patched (explicitFileType=sourcecode.cpp.objcpp), skip")
            return
        if not content.contains(old_str):
            push_warning("[UniKitIOSExporter] project.pbxproj 中找不到 dummy.cpp 的 lastKnownFileType=sourcecode.cpp.cpp,Godot 模板可能已变化,xcodebuild archive 可能因 ObjC++ 注入而失败")
            return
    ...
    func _patch_code_sign_identity_for_automatic_signing() -> void :
        ...
        if not content.contains(old_str):
            ...
            print("  CODE_SIGN_IDENTITY=\"Apple Distribution\" not found, skip (already patched or template changed)")
            return
        ...
        f.store_string(content)
        f.close()
        print("  patched %d occurrence(s) of CODE_SIGN_IDENTITY \"Apple Distribution\" → \"Apple Development\"" % occurrences)
    ...
    func _patch_entitlements() -> void :
        ...
        if not FileAccess.file_exists(ent_path):
            if not _create_empty_entitlements(ent_path):
                ...
                return
            created_now = true
            print("  [create] 创建空 entitlements 文件 (Godot 未自动生成,export_presets entitlements/* 全 Disabled)")
            if not _patch_pbxproj_set_entitlements(ent_relpath):
                push_warning("[UniKitIOSExporter] patch pbxproj CODE_SIGN_ENTITLEMENTS 失败,Xcode 不会签名 entitlements,LUID 仍可能不生效")
        else:
        ...
        var groups_xml: String = "<array>%s</array>"\
%_value_to_plist_xml("$(AppIdentifierPrefix)com.learningsSharingGroup")
        var rm_out: Array = []
        var rm_rc: int = OS.execute("plutil", ["-remove", "keychain-access-groups", ent_path], rm_out, true)
        var ins_out: Array = []
        var ins_rc: int = OS.execute("plutil", ["-insert", "keychain-access-groups", "-xml", groups_xml, ent_path], ins_out, true)
        if ins_rc != 0:
            push_error("[UniKitIOSExporter] plutil -insert keychain-access-groups 失败 rc=%d output=[%s]"
                %[ins_rc, _flat_lines(ins_out)])
            return
        print("  ✓ keychain-access-groups = [\"$(AppIdentifierPrefix)com.learningsSharingGroup\"] (created_now=%s, remove rc=%d, insert rc=%d)"
        ...
        var verify_rc: int = OS.execute("/usr/libexec/PlistBuddy", ["-c", "Print :keychain-access-groups:0", ent_path], verify_out, true)
        if verify_rc == 0:
            ...
            if v != "$(AppIdentifierPrefix)com.learningsSharingGroup":
                push_error("[UniKitIOSExporter] keychain-access-groups[0] 不匹配预期: expect=\"$(AppIdentifierPrefix)com.learningsSharingGroup\" got=\"%s\"" % v)
        else:
            push_error("[UniKitIOSExporter] verify keychain-access-groups[0] 失败 rc=%d output=[%s]"
    ...
    func _create_empty_entitlements(path: String) -> bool:
        ...
        if f == null:
            push_error("[UniKitIOSExporter] 创建 entitlements 文件失败: %s err=%s"
    ...
    func _patch_pbxproj_set_entitlements(ent_relpath: String) -> bool:
        ...
        if content.contains("CODE_SIGN_ENTITLEMENTS"):
            print("  [pbxproj] CODE_SIGN_ENTITLEMENTS 已存在 (Godot 自动生成或上次 patch 残留),跳过 pbxproj patch")
            return true
        ...
        if matches.is_empty():
            push_warning("[UniKitIOSExporter] project.pbxproj 找不到 INFOPLIST_FILE = \"...\"; anchor,"
        ...
        f.store_string(content)
        f.close()
        print("  [pbxproj] 在 %d 处 INFOPLIST_FILE 后追加 CODE_SIGN_ENTITLEMENTS = \"%s\""
    ...
    func _patch_one_key(plist_path: String, key: String, value: Variant) -> bool:
        ...
        var ins_out: Array = []
        var ins_rc: int = OS.execute("plutil", ["-insert", key, "-xml", xml, plist_path], ins_out, true)
        if ins_rc != 0:
            push_error("[UniKitIOSExporter]     ✗ plutil -insert :%s 失败 rc=%d output=[%s] xml_preview=%s"
    ...
    func _list_plist_top_keys(plist_path: String) -> Array:
        ...
        if rc != 0:
            push_warning("[UniKitIOSExporter] _list_plist_top_keys: plutil -convert json 失败 rc=%d output=[%s]"
                %[rc, _flat_lines(out)])
            return []
        var json_str: String = "\n".join(out)
        var parsed: Variant = JSON.parse_string(json_str)
        if typeof(parsed) != TYPE_DICTIONARY:
            push_warning("[UniKitIOSExporter] _list_plist_top_keys: JSON 解析失败 (%s 字节)" % json_str.length())
            return []
    ...
    func _read_plist_string(plist_path: String, key: String) -> String:
        var out: Array = []
        var rc: int = OS.execute("/usr/libexec/PlistBuddy", ["-c", "Print :%s" % key, plist_path], out, true)
        if rc != 0:
            return ""
    ...
    func _register_google_service_plist() -> void :
        ...
        if not FileAccess.file_exists(src):
            push_warning("[UniKitIOSExporter] GoogleService-Info.plist 不存在,Firebase 不可用。请从 Firebase 控制台下载,放到 ios/build/(模板见 .example)")
            return
    ...
    func _register_abtest_config() -> void :
        ...
        for name in names:
            ...
            if FileAccess.file_exists(src):
                ...
                registered = true
        if not registered:
            push_warning("[UniKitIOSExporter] ABTest.json / ABTest_Debug.json 均不存在,本地分流 AB 参数将回落默认。"
    ...
    func _register_splash_overlay_images() -> void :
        ...
        if ok_2x or ok_3x:
            ...
            push_warning("[UniKitIOSExporter] SplashImage 启动图未找到,iOS 启动时可能出现白屏。"
    ...
    func _print_pod_install_hint() -> void :
        ...
        print("   ./Pods/LearningsAdsKit/releases/LearningsAdsKit/MeeviiAdsHelper/meeviiads install \\")
