...
func _max_duration() -> float:
	var d := 0.0
	if rotation_enabled:
		d = maxf(d, rotation_duration)
	if translation_enabled:
		d = maxf(d, translation_duration)
	if scale_enabled:
		d = maxf(d, scale_duration)
	if alpha_enabled:
		d = maxf(d, alpha_duration)
	return d
...
func _apply_all(elapsed: float) -> void:
	...
	if rotation_enabled:
		var t := _ease_t(
			clampf(elapsed / rotation_duration, 0.0, 1.0), rotation_curve, rotation_custom_curve
		)
		var rot := lerpf(_origin_rot, _origin_rot + rotation_angle, t)
		if p is Node2D:
			(p as Node2D).rotation_degrees = rot
		elif p is Control:
			(p as Control).rotation_degrees = rot

	if translation_enabled:
		var tx := _ease_t(
			clampf(elapsed / translation_duration, 0.0, 1.0),
			translation_curve_x,
			translation_custom_curve_x
		)
		var ty := _ease_t(
			clampf(elapsed / translation_duration, 0.0, 1.0),
			translation_curve_y,
			translation_custom_curve_y
		)
		var pos := Vector2(
		...
		if p is Node2D:
			(p as Node2D).position = pos
		elif p is Control:
			(p as Control).position = pos

	if scale_enabled:
		var tsx := _ease_t(
			clampf(elapsed / scale_duration, 0.0, 1.0), scale_curve_x, scale_custom_curve_x
		)
		var tsy := _ease_t(
			clampf(elapsed / scale_duration, 0.0, 1.0), scale_curve_y, scale_custom_curve_y
		)
		var scl := Vector2(
			lerpf(_origin_scale.x, scale_target_x, tsx), lerpf(_origin_scale.y, scale_target_y, tsy)
		)
		if p is Node2D:
			(p as Node2D).scale = scl
		elif p is Control:
			(p as Control).scale = scl

	if alpha_enabled and p is CanvasItem:
		var ta := _ease_t(
			clampf(elapsed / alpha_duration, 0.0, 1.0), alpha_curve, alpha_custom_curve
		)
		var col: Color = (p as CanvasItem).modulate
