...
const TUTORIAL_SOLUTION: Array[Vector2i] = [
    Vector2i(0, 2), Vector2i(1, 0), Vector2i(2, 3), Vector2i(3, 1), 
]
...
func _play_iq_feedback() -> void :
    ...
    grow.tween_method(_set_iq_fill_right, _iq_fill_right(float(old_v)), _iq_fill_right(float(new_v)), 0.4)\
...
func _step_3_mark_row_col() -> void :
    ...
    var cells: Array[Vector2i] = [
        Vector2i(0, 0), Vector2i(0, 1), Vector2i(0, 3), 
...
func _step_5_mark_neighbors() -> void :
    var step3_key: String = "TUTORIAL_STEP3_RICH_DIAGONAL" if ABTestManager.tutorial_diagonal.is_diagonal_copy() else "TUTORIAL_STEP3_RICH"
...
func _step_6_place_third_cat() -> void :
    ...
    var mirror_cells: Array[Vector2i] = [
        Vector2i(0, 0), Vector2i(2, 0), Vector2i(2, 1), Vector2i(3, 0), 
    ]
    _allowed_cells = [target]
    _clear_mask_hint_cells()
    _show_mask_hints(hint_cells, mirror_cells)
...
func _start_swipe_hand_loop(cells: Array[Vector2i]) -> void :
    ...
    for cell in cells:
        offsets.append(Vector2(
            111.0 + (cell.y - 2) * slot_screen, 
            -316.0 + (cell.x - 0) * slot_screen
        ))
    var tw: = create_tween()
    tw.set_loops()
...
func _fade_out_mask_layer() -> void :
    ...
    _mask_tween.tween_callback( func() -> void :
        _mask_layer.visible = false
        _mask_layer.modulate.a = 1.0
        if _iq_bar.visible:
...
func _spawn_confetti() -> void :
    var colors: Array[Color] = [
        Color("#FF5252"), Color("#448AFF"), Color("#69F0AE"), 
        Color("#FFD740"), Color("#FF4081"), Color("#40C4FF"), 
    ]
    for _i in range(30):
        ...
        add_child(rect)
        var tw: = create_tween()
        tw.tween_property(rect, "position:y", 1980.0, randf_range(2.0, 3.5))\
...
func _on_board_cell_drag_start(pos: Vector2) -> void :
    ...
    if _current_mode == StepMode.FREE_PLAY\
and (_step7_hint_phase == 0 or _step7_hint_phase == 3):
...
func _color_name_bbcode_for_cell(r: int, c: int) -> String:
    var region_idx: int = _guide_regions[r][c] if r < _guide_regions.size() and c < _guide_regions[r].size() else -1
    if region_idx < 0:
    ...
    if ABTestManager.region_color.value() == RegionColorConfig.VALUE_CONTROL:
        ...
        var hex_codes: Array[String] = ["#D980A4", "#BC537C", "#8465D6", "#F970DE", "#FFAA6E", "#DDA916", "#49658F", "#83AAD2", "#3497CB", "#289692", "#78AB5A", "#AC6F48"]
