...
@onready var _revive_text_icon_row: HBoxContainer = $Root / Ctrl / VBoxContainer / ReviveBtn / Root / TextIconRow
...
func _refresh_dynamic_text() -> void :
    ...
    var key: String = "FAIL_TITLE_FISH" if ABTestManager.life_icon.is_fish_life_bar() else "FAIL_TITLE"
    ...
    var main_key: String = "FAIL_REVIVE_3FISH" if ABTestManager.revive_life.is_alt_button_text() else "FAIL_REVIVE"
...
func _fit_main_label_width() -> void :
    ...
    if measured <= avail:
        return
    var new_size: int = maxi(_REVIVE_MAIN_FONT_SIZE_MIN, int(floor(_REVIVE_MAIN_FONT_SIZE_BASE * avail / measured)))
...
func show_fail(level_config: Dictionary, retry_params: Dictionary = {}, remaining_cats: int = 0) -> void :
    _level_config = level_config
    _retry_params = retry_params
    _count_num.text = str(remaining_cats)
    _refresh_dynamic_text()
    var pos: String = Tracker.AdPos.DAILY_GAME_FAIL if _level_config.get("is_daily", false) else Tracker.AdPos.NORMAL_GAME_FAIL
...
func _on_restart_btn_pressed() -> void :
    ...
    var params: Dictionary = _retry_params.duplicate() if not _retry_params.is_empty()\
