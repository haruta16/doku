...
static func eval_scheme(scheme: String) -> Dictionary:
    ...
    if scheme.begins_with(SCHEME_SESSION_GAME_PREFIX):
        return _eval_count(scheme, SCHEME_SESSION_GAME_PREFIX, GameState.get_session_played_count(), "本 session 完局数")
    if scheme.begins_with(SCHEME_DAY_GAME_PREFIX):
        return _eval_count(scheme, SCHEME_DAY_GAME_PREFIX, GameState.get_today_played_count(), "今日完局数")
    if scheme.begins_with(SCHEME_FIRST_DAY_PREFIX):
        return _eval_first_day(scheme)
    if scheme.begins_with(SCHEME_DAY_MIN_PREFIX):
        return _eval_minutes(scheme, SCHEME_DAY_MIN_PREFIX, SessionManager.get_today_active_sec(), "今日前台活跃")
    if scheme.begins_with(SCHEME_SESSION_MIN_PREFIX):
        return _eval_minutes(scheme, SCHEME_SESSION_MIN_PREFIX, SessionManager.get_session_active_sec(), "本 session 前台活跃")

    return {"blocked": false, "reason": ""}


static func _eval_count(scheme: String, prefix: String, played_count: int, label: String) -> Dictionary:
    var n: int = _parse_n(scheme, prefix)
    if n < 0:
        return {"blocked": false, "reason": ""}
    if played_count < (n - 1):
        return {"blocked": true, "reason": "%s %d < 门槛 %d(%s)" % [label, played_count, n - 1, scheme]}
    return {"blocked": false, "reason": ""}




static func _eval_minutes(scheme: String, prefix: String, active_sec: int, label: String) -> Dictionary:
    var n: int = _parse_n(scheme, prefix)
    if n < 0:
        return {"blocked": false, "reason": ""}
    var threshold_sec: int = n * 60
    if active_sec < threshold_sec:
        return {"blocked": true, "reason": "%s %d 秒 < 门槛 %d 秒(%s)" % [label, active_sec, threshold_sec, scheme]}
    return {"blocked": false, "reason": ""}
