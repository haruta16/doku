...
func _process_connection(conn: Dictionary) -> bool:
    ...
    if ctx == null:
        _write_response(peer, _build_response_string(400, "{\"result\":\"error\",\"message\":\"Bad Request\"}"))
        return false
...
func _dispatch(ctx: DebugApiContext) -> void :
    if not _routes.has(ctx.path):
        ctx.status_code = 404
        ctx.response_body = JSON.stringify({"result": "error", "message": "Not Found: %s" % ctx.path})
        return
