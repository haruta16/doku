...
func _compute_ab_size(level_num: int, sc: int) -> int:
    match sc:
        3:
            if level_num <= 10: return _SIZE_CYCLE_A_1_10[level_num - 1]
            if level_num <= 20: return _SIZE_CYCLE_A_11_20[level_num - 11]
            return _SIZE_CYCLE_A_21_PLUS[(level_num - 21) % 10]
        4:
            if level_num <= 10: return _SIZE_CYCLE_B_1_10[level_num - 1]
            if level_num <= 20: return _SIZE_CYCLE_B_11_20[level_num - 11]
            if level_num <= 50: return _SIZE_CYCLE_B_21_50[(level_num - 21) % 10]
            return _SIZE_CYCLE_B_51_PLUS[(level_num - 51) % 10]
        5:
            if level_num <= 10: return _SIZE_CYCLE_C_1_10[level_num - 1]
            if level_num <= 20: return _SIZE_CYCLE_C_11_20[level_num - 11]
            if level_num <= 100: return _SIZE_CYCLE_C_21_100[(level_num - 21) % 10]
            return _SIZE_CYCLE_C_101_PLUS[(level_num - 101) % 10]
        6:
            if level_num <= 10: return _SIZE_CYCLE_D_1_10[level_num - 1]
            return _SIZE_CYCLE_D_11_PLUS[(level_num - 11) % 10]
        7:
            if level_num <= 10: return _SIZE_CYCLE_E_1_10[level_num - 1]
            if level_num <= 20: return _SIZE_CYCLE_E_11_20[level_num - 11]
            if level_num <= 50: return _SIZE_CYCLE_E_21_50[(level_num - 21) % 10]
            return _SIZE_CYCLE_E_51_PLUS[(level_num - 51) % 10]
        8:
            if level_num <= 10: return _SIZE_CYCLE_F_1_10[level_num - 1]
            if level_num <= 50: return _SIZE_CYCLE_F_11_50[(level_num - 11) % 10]
            return _SIZE_CYCLE_F_51_PLUS[(level_num - 51) % 10]
        _:
            if level_num <= 10: return _SIZE_CYCLE_CTRL_1_10[level_num - 1]
            if level_num <= 20: return _SIZE_CYCLE_CTRL_11_20[level_num - 11]
            if level_num <= 50: return _SIZE_CYCLE_CTRL_21_50[(level_num - 21) % 10]
...
func _on_start_pressed() -> void :
    ...
    if summary.get("ok", false):
        _result_label.text = "完成:%d 局\n唯一 pid 前缀:%d\n跨 level 重复组:%d\ndedup 上报合计:%d\n  ├─ 已知池小(噪声):%d\n  └─ **未知根因(focus):%d**\n报告:%s" % [
            count, summary["unique_count"], summary["cross_lv_dup_groups"], 
            summary["dedup_report_count"], 
            summary["dedup_known_count"], summary["dedup_unknown_count"], 
            summary["report_path"]
        ]
...
func _build_record_meta(iter_idx: int, lv: int, sz: int, source: String, pid: String, 
        entry: Dictionary, rm: Array, actions: PackedStringArray) -> Dictionary:
    return {
        "iter": iter_idx, 
        "lv": lv, 
        "sz": sz, 
...
static func _is_known_pool_small_dup(r: Dictionary, pool_sizes: Dictionary) -> bool:
    ...
    if int(r.get("prev_sz", 0)) != int(r.get("curr_sz", 0)): return false
    if int(r.get("prev_rank", 0)) != int(r.get("curr_rank", 0)): return false
    if String(r.get("prev_tier", "")) != String(r.get("curr_tier", "")): return false
    if String(r.get("prev_bank_source_main", "")) != String(r.get("curr_bank_source_main", "")): return false
...
func _run_simulation(count: int) -> Dictionary:
    ...
    for i in range(count):
        ...
        if sz == 0: sz = (entry["regionMap"] as Array).size()
        ...
        var curr_meta: Dictionary = _build_record_meta(i + 1, lv, sz, source, full_pid, entry, rm, actions)
        ...
        if not dup_prev.is_empty() and int(dup_prev.get("level", -1)) != lv:
            ...
            if not retry_entry.is_empty() and retry_entry.has("regionMap"):
                ...
                if retry_sz == 0: retry_sz = (retry_entry["regionMap"] as Array).size()
                var retry_rm: Array = retry_entry["regionMap"]
                var retry_pid: String = LevelData.compute_puzzle_id(retry_sz, retry_rm)
                var retry_meta: Dictionary = _build_record_meta(i + 1, lv, retry_sz, source + "/retry", retry_pid, retry_entry, retry_rm, actions)
                ...
                if not retry_dup.is_empty() and int(retry_dup.get("level", -1)) != lv:
                    ...
                    var prev_meta: Dictionary = by_pid_full_record.get(retry_pid, {
                        "iter": -1, "lv": int(retry_dup.get("level", -1)), 
                        "source": "?", "region_map_sha256": "?", 
                        "bank_source": "?", "bank_source_main": "?", 
                        "bank_idx": -1, "rank": -1, "tier": "?", 
                        "strategy": -1, "rnr": -1, 
                        "is_dfe": false, "is_hard": false, 
                        "actions": "?", 
                    })
                    ...
                    dedup_entry["bucket_pool_size_curr"] = _bucket_pool_size("curr", dedup_entry, pool_sizes)
                    dedup_entry["bucket_pool_size_prev"] = _bucket_pool_size("prev", dedup_entry, pool_sizes)
        ...
        log_lines.append("[%05d] lv=%-4d pid=%-37s source=%-13s actions=%s%s" % [
            i + 1, lv, full_pid, source, ",".join(actions), dedup_mark
        ])
        if not by_prefix.has(prefix): by_prefix[prefix] = []
        by_prefix[prefix].append({
            "iter": i + 1, "lv": lv, "source": source, "actions": ",".join(actions), 
        })
    ...
    var report_path: String = _write_report(count, log_lines, by_prefix, cross_lv_dups, window_dups, dedup_reports)
    ...
    for r in dedup_reports:
        if bool(r.get("is_known_pool", false)): dedup_known_count += 1
...
static func _append_dedup_detail(out: PackedStringArray, r: Dictionary) -> void :
    out.append("### [%05d → %05d] iter_distance=%d  pid=`%s`" % [
    ...
    out.append("| bank_source_main | %s | %s |" % [r["prev_bank_source_main"], r["curr_bank_source_main"]])
    ...
    out.append("| bucket pool size | %d | %d |" % [int(r.get("bucket_pool_size_prev", -1)), int(r.get("bucket_pool_size_curr", -1))])
    out.append("| actions | %s | %s |" % [r["prev_actions"], r["curr_actions"]])
    out.append("")
    out.append("CURR 时上下文:strategy=%d, rnr=%d, is_dfe=%s, is_hard=%s" % [
...
func _write_report(count: int, log_lines: Array, by_prefix: Dictionary, cross_lv_dups: Array, window_dups: Array, dedup_reports: Array) -> String:
    ...
    for r in dedup_reports:
        if bool(r.get("is_known_pool", false)): dedup_known.append(r)
        else: dedup_unknown.append(r)
    ...
    out.append("")
    out.append("已剔除「PREV / CURR 同桶且该桶池 ≤ %d」的案例(那些都属已定位的池小循环根因)。**剩余即与线上 session 3/4/5 同类的待查问题**:跨桶撞重 / 大桶撞重 / idx 推进异常等。" % POOL_SMALL_THRESHOLD)
    out.append("")
    if dedup_unknown.is_empty():
        out.append("**无**(本轮模拟未触发任何「非池小」路径的 dedup 上报)")
        out.append("")
    else:
        dedup_unknown.sort_custom( func(a, b): return int(a["iter_distance"]) < int(b["iter_distance"]))
    ...
    out.append("")
    out.append("PREV / CURR 同桶且桶内题数 ≤ %d → 必然池小循环,根因已定位(LKModified 各桶 ≤ 38 / 主线 sz=10 R1=14 / sz=4-5 / sz=11 等小桶)。" % POOL_SMALL_THRESHOLD)
    ...
    if dedup_known.is_empty():
        ...
        for r in dedup_known:
            ...
            if main == "lk_mod":
                key = "lkmod_%d_R%d (池=%d)" % [int(r["curr_sz"]), int(r["curr_rank"]), int(r["bucket_pool_size_curr"])]
            else:
                key = "main_%d_R%d_%s (池=%d)" % [int(r["curr_sz"]), int(r["curr_rank"]), 
                    String(r.get("curr_tier", "")) if String(r.get("curr_tier", "")) != "" else "noT", 
        ...
        out.append("")
        dedup_known.sort_custom( func(a, b): return int(a["iter_distance"]) < int(b["iter_distance"]))
    ...
    if cross_lv_dups.is_empty():
        ...
        cross_lv_dups.sort_custom( func(a, b): return (a["hits"] as Array).size() > (b["hits"] as Array).size())
        var n: int = min(50, cross_lv_dups.size())
        for i in range(n):
            var g: Dictionary = cross_lv_dups[i]
            out.append("### prefix=`%s`  unique_lvs=%s  total_hits=%d" % [g["prefix"], str(g["unique_lvs"]), (g["hits"] as Array).size()])
            for h in g["hits"]:
                out.append("  - [%05d] lv=%d source=%s actions=%s" % [h["iter"], h["lv"], h["source"], h["actions"]])
            out.append("")
    ...
    if window_dups.is_empty():
        ...
        for i in range(nw):
            ...
            out.append("- prefix=`%s` [%05d]lv=%d → [%05d]lv=%d (间隔 %d 关)" % [
                d["prefix"], d["a"]["iter"], d["a"]["lv"], d["b"]["iter"], d["b"]["lv"], 
                absi(d["b"]["lv"] - d["a"]["lv"]), 
            ])
    ...
    out.append("")
    out.append("```")
    for line in log_lines: out.append(line)
    out.append("```")

    var f: FileAccess = FileAccess.open(path, FileAccess.WRITE)
    if f == null: return "(写文件失败:%s)" % path
    f.store_string("\n".join(out))
    f.close()
    return path
