...
func _on_start_pressed() -> void :
    ...
    var sz: int = int(entry["size"]) if entry.has("size") else int((entry["regionMap"] as Array).size())
    var rank: int = int(entry.get("r", 0))
    var idx: int = int(entry.get("id", 0))

    var sol_1d: Array = []
    for v in entry["solution"]: sol_1d.append(int(v))
    ...
    print("[level_json] start sz=%d rank=%d id=%d cats=%d marks=%d errors=%d" % [sz, rank, idx, placed_cats.size(), marks.size(), errors.size()])
    UIManager.hide_ui(UiName.LEVEL_JSON_INPUT)
...
func _validate(entry: Dictionary) -> String:
    if not entry.has("regionMap"): return "缺少字段:regionMap"
    if not entry.has("solution"): return "缺少字段:solution"
