...
func _ready() -> void :
    ...
    if UniKitManager.is_analyze_inited():
        ...
        UniKitManager.analyze_init_completed.connect(_emit_perf.bind(Tracker.PerfStep.INIT_GAME_MANAGER), CONNECT_ONE_SHOT)
    if UniKitManager.is_ad_inited():
        _emit_perf(Tracker.PerfStep.INIT_AD_MANAGER)
    else:
        UniKitManager.ad_init_completed.connect(_emit_perf.bind(Tracker.PerfStep.INIT_AD_MANAGER), CONNECT_ONE_SHOT)
...
func _prewarm_game() -> void :
    ...
    var sz: int = LevelData.get_size_group_j(lv) if ABTestManager.rule_normal_rank.is_group_j() else LevelData.get_size(lv)
...
func _register_new_pushes() -> void :
    ...
    for i: int in range(1, 101):
        noon_all.append({"title": tr("PUSH_NOON_TITLE_%d" % i), "content": tr("PUSH_NOON_BODY_%d" % i)})
    noon_all.shuffle()
    var evening_all: Array[Dictionary] = []
    for i: int in range(1, 101):
        evening_all.append({"title": tr("PUSH_EVE_TITLE_%d" % i), "content": tr("PUSH_EVE_BODY_%d" % i)})
    evening_all.shuffle()
...
func _wait_cmp_then_att_max_2s() -> void :
    ...
    if (shown_guide or att_status != UniKitManager.ATT_STATUS_NOT_DETERMINED) and not UniKitManager.debug_force_editor_test:
    ...
    UniKitManager.check_cmp( func() -> void :
        state.cmp_done = true
