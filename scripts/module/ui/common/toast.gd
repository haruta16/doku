...
func _play(msg: String) -> void :
    ...
    if line_count == 1:
        ...
        var ms: Vector2 = font.get_multiline_string_size(_label.text, HORIZONTAL_ALIGNMENT_LEFT, wrap_w, fs)
    ...
    tw_pos.tween_property(_panel, "position:y", 
        start_y - _FLOAT_DIST, 1.55).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
