...
func _compact_z_indices(layer: int) -> void :
    ...
    visible_wins.sort_custom( func(a: UIFrameWindow, b: UIFrameWindow) -> bool:
...
func block_input_briefly(target: Control, duration: float = 1.5) -> void :
    ...
    target.get_tree().create_timer(duration).timeout.connect( func() -> void :
...
func _hide_mask() -> void :
    ...
    if _mask_ref_count <= 0:
        ...
        if _mask_secondary != null and is_instance_valid(_mask_secondary) and _mask_secondary.visible:
...
func _abort_mask_crossfade() -> void :
    if _mask_secondary == null or not is_instance_valid(_mask_secondary) or not _mask_secondary.visible:
        return
    _kill_mask_tween()
    _kill_mask_secondary_tween()
    var old_primary: = _mask
...
func _start_mask_crossfade(closing_win: UIFrameWindow, duration: float) -> void :
    ...
    _mask_secondary_tween.tween_property(_mask_secondary, "color:a", next_top.mask_opacity, duration)
...
const _SAFE_AREA_PAGES: Array[StringName] = [
    &"home", &"game", &"daily_game", &"win", &"fail", &"setting", &"bank", 
]
    ...
    for n in node.find_children("*", "Control", true, false):
        ...
        if c.is_in_group(_COLLAPSE_WHEN_TOP_SAFE_GROUP):
            if not c.has_meta(_COLLAPSE_BASELINE_META):
                c.set_meta(_COLLAPSE_BASELINE_META, 
