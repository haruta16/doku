...
func on_tap(r: int, c: int, stroke: BoardStrokeContext) -> Array[CellAction]:
    ...
    if CellState.is_blank(cur):
        out.append(CellAction.set_cell(r, c, CellState.EMPTY, CellState.MARK, VibrateManager.Level.LEVEL2))
    elif cur == CellState.MARK:
        out.append(CellAction.set_cell(r, c, CellState.MARK, CellState.EMPTY, VibrateManager.Level.LEVEL2))
    return out
