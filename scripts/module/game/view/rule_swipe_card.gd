...
func _fill_content(content: Control, rule_idx: int) -> void :
    ...
    var text_w: float = ceil(font.get_multiline_string_size(disp, HORIZONTAL_ALIGNMENT_CENTER, _LABEL_MEASURE_W, fs).x)
...
func _go_to(target: int) -> void :
    ...
    _tween.tween_property(out_layer, "position:x", - float(dir) * _DX, _OUT_SEC)\
.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN_OUT)

    _tween.tween_property(in_layer, "position:x", overshoot, _OUT_SEC)\
.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN_OUT)

    _tween.tween_property(in_layer, "position:x", 0.0, _IN_BACK_SEC)\
.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN_OUT).set_delay(_OUT_SEC)
