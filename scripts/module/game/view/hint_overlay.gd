    ...
    match strategy:
        ...
        _:
            ...
            if desc_override != "":
                ...
                match unit_type:
                    "row": unit_name = tr("HINT_ROW") % (hint.get("unit_index", 0) + 1)
                    "col": unit_name = tr("HINT_COL") % (hint.get("unit_index", 0) + 1)
                    "region": unit_name = tr("HINT_COLOR_REGION")
                    _: unit_name = tr("HINT_UNIT_ROW_COL_REGION")
                _desc_label.clear()
                _desc_label.append_text(tr("HINT_ONLY_ONE_CELL") % unit_name)

    var _chain_d: Dictionary = hint.get("chain", {})
    var has_steps: = (strategy == "R4_chain" or strategy == "R5_chain")\
and not _chain_d.is_empty() and (_chain_d.get("steps", []) as Array).size() > 0
