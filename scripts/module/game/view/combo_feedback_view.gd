...
const SCORE_BUBBLE_SCENE: PackedScene = preload("res://scripts/module/game/ui/compont/level_flow_score.tscn")
const ENCOURAGE_SCENE: PackedScene = preload("res://scripts/module/game/ui/compont/level_encourage.tscn")
...
const ENCOURAGE_HALF_WIDTHS: Array[float] = [
    128.0, 149.0, 180.0, 220.0, 212.0, 225.0, 
]
...
func show_combo(combo_count: int, cell_global_pos: Vector2, total_score: int, gain: int) -> void :
    ...
    if ABTestManager.combo_encourage.is_follow_cat():
        if has_score:
            _show_encourage_at_position(combo_count, cell_global_pos, base_off + ENCOURAGE_Y_OFFSET_ABOVE_SCORE)
        else:
...
func _play_combo_feedback_voice(combo_count: int) -> void :
    if ABTestManager.combo_voice.is_enabled():
        SoundManager.play_combo_voice_by_path(ABTestManager.combo_voice.get_combo_voice(combo_count))
        return
...
func _show_score_bubble(global_pos: Vector2, gain: int) -> void :
    ...
    bubble.position = global_pos - global_position - Vector2(bubble.size.x * 0.5, _score_bubble_y_offset())
