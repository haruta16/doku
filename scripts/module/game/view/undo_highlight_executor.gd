...
func execute(step: StepHistory.StepRecord, mode: int, duration: float) -> void :
    ...
    _outline_renderer.show_outline(
        _highlight_cells, 
        BoardView.CELL_PX, 
...
func _play_appear_scale() -> void :
    ...
    _scale_tween.tween_method(_apply_per_cluster_scale, 1.0, 1.05, 0.117).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
    _scale_tween.tween_method(_apply_per_cluster_scale, 1.05, 1.0, 0.166).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_QUAD)
