...
func notify_group_dyed() -> void :
    ...
    print("[StreakSwitch] notify_group_dyed: cur=%d last_group=%d pending=%d" % [cur, _data.last_group, _data.pending_switch_page])
    ...
    print("[StreakSwitch] notify: 切组 %d->%d 映射 page=%d pending=%d eligible=%s" % [_data.last_group, cur, page, _data.pending_switch_page, str(_pending_switch_eligible)])
...
func do_checkin() -> Dictionary:
    ...
    if _data.reward_cycle_day > 0 and _data.reward_cycle_day % CYCLE_LENGTH == 0:
        if ABTestManager.daily_streak.has_reward():
            ...
            var items: Array = _build_reward_items()
            var uid: int = AwardManager.dispatch(
                items, 
...
func get_week_slots() -> Array[Dictionary]:
    ...
    var start_wd: int = _data.streak_start_weekday if _data.streak_start_weekday >= 0 else _today_weekday()
    var filled: int = 0
