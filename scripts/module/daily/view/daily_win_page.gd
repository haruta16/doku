...
%[tr("DAILY_WIN_TIME"), m, s]
    ...
    var highlighted: String = "[color=#02be52][font_size=90][b]%s[/b][/font_size][/color]" % pct_text
    var beat_bbcode: String = "[color=#ffe375]" + beat_str.replace(pct_text, "[/color]" + highlighted + "[color=#ffe375]") + "[/color]"
    _beat_label.text = ""
    ...
    if not level_config.get("toast_was_shown", false) and not level_config.get("skip_appear_delay", false):
