...
@onready var _revive_ad_icon: TextureRect = $Root / BottomGroup / VBoxContainer / ReviveBtn / Root / TextIconRow / Icon
@onready var _revive_badge: GameAdBadge = $Root / BottomGroup / VBoxContainer / ReviveBtn / Root / Badge


@onready var _revive_text_icon_row: HBoxContainer = $Root / BottomGroup / VBoxContainer / ReviveBtn / Root / TextIconRow
@onready var _revive_main_label: Label = $Root / BottomGroup / VBoxContainer / ReviveBtn / Root / TextIconRow / Label
...
func show_fail(level_config: Dictionary, remaining_cats: int = 0) -> void :
    ...
    _revive_btn.visible = true if free else UniKitManager.is_reward_valid("reward", Tracker.AdPos.DAILY_GAME_FAIL)
