...
@onready var _timer_label: Label = $Root / VBoxContainer / CatHeartRow / TimeContainer / HBoxContainer / TimerCtrl / TimerLabel
...
func on_show(params: Dictionary = {}) -> void :
    ...
    _start_date = "%d-%02d-%02d" % [dt_local.get("year", 2026), dt_local.get("month", 1), dt_local.get("day", 1)]
    ...
    if DailyGamePage.debug_day_override >= 0:
        ...
        day_offset = max(0, _local_date_to_jdn(
    ...
    while attempts < total_virtual:
        ...
        if QueendokuCore.validate_solution_entry({"regionMap": int_regions, "solution": sol_1d}, sz):
            break
        push_error("DailyGamePage: invalid solution day_offset=%d virtual_idx=%d transform=%d entry_idx=%d, advancing"\
%[day_offset, virtual_idx, transform, entry_idx])
    ...
    if board_intro != null and not board_intro.animation_finished.is_connected(_on_appear_animation_finished):
    ...
    var color_map: Array[int] = LevelGenerator.compute_color_map_with_seed(sz, _puzzle["regions"], color_seed)
...
func _on_game_over() -> void :
    ...
    var fail: = UIManager.show_ui(UiName.DAILY_FAIL, {
...
func _on_game_complete() -> void :
    ...
    var raw_percent: float = DailyStats.beat_percent(elapsed_sec, rank, _level_config.get("size", 12))


    if ABTestManager.game_auto_mark.is_prop_ad_mode() and GameState.is_daily_auto_mark_enabled_for_today():
    ...
    var streak_consumed_appear_delay: bool = await _try_run_streak_flow_after_win(&"challenge", toast_was_shown)
...
func _build_game_end_params(result: String) -> Dictionary:
    ...
    var params: Dictionary = {
        ...
        "locate_used_total": GameState.get_game_total_stat(Tracker.GameType.DAILY, "locate_used_total"), 
        ...
        "clear_used_total": GameState.get_game_total_stat(Tracker.GameType.DAILY, "clear_used_total"), 
        "draft_used_total": GameState.get_game_total_stat(Tracker.GameType.DAILY, "draft_used_total"), 
        "draft_time_total": int(GameState.get_game_total_stat(Tracker.GameType.DAILY, "draft_time_total_ms") / 1000.0), 
        "draft_error_total": GameState.get_game_total_stat(Tracker.GameType.DAILY, "draft_error_total"), 
        "draft_correct_total": GameState.get_game_total_stat(Tracker.GameType.DAILY, "draft_correct_total"), 
        ...
        "invalid_sign_total": GameState.get_game_total_stat(Tracker.GameType.DAILY, "invalid_sign_total"), 
        "fail_sign": sz - cat_n, 
