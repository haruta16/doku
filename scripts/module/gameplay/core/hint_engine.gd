...
static func find_r1_hint(board: Array, size: int, regions: Array) -> Dictionary:
    ...
    var row_piece: Array[bool] = [];row_piece.resize(size);row_piece.fill(false)
    var col_piece: Array[bool] = [];col_piece.resize(size);col_piece.fill(false)
    var reg_piece: Array[bool] = [];reg_piece.resize(size);reg_piece.fill(false)
    ...
    for r in range(size):
        ...
        for c in range(size):
            ...
            if col_uniform:
                ...
                return {
                    "found": true, "cell": Vector2i(r, c), 
                    "unit_type": "full_line", "unit_index": row_reg, 
                    "unit_cells": unit
                }
    ...
    for r in range(size):
        ...
        if cands.size() == 1:
            return {
                "found": true, "cell": cands[0], 
                "unit_type": "row", "unit_index": r, 
    ...
    for reg in range(size):
        ...
        if cands.size() == 1:
            return {
                "found": true, "cell": cands[0], 
                "unit_type": "region", "unit_index": reg, 
                "unit_cells": unit
            }
...
static func find_r3_r4_hint(board: Array, size: int, regions: Array) -> Dictionary:
    ...
    for reg in range(size):
        ...
        for r in range(size):
            for c in range(size):
                if regions[r][c] == reg and _can_place(r, c, board, size, regions, row_piece, col_piece, reg_piece):
                    rows[r] = true
                    cols[c] = true
        reg_rows[reg] = rows
        reg_cols[reg] = cols
        ...
        for subset in subsets:
            ...
            if valid_r and all_rows.size() == k:
                ...
                if has_new:
                    ...
                    return {
                        "found": true, "strategy": strat, 
                        "description": TranslationServer.translate("HINT_R3_ROW") % [k, k, k], 
                        "regions": subset, "highlight_cells": hl_cells, 
                        "locked_rows": all_rows.keys(), "locked_cols": []
                    }
            ...
            if valid_c and all_cols.size() == k:
                ...
                if has_new:
                    ...
                    return {
                        ...
                        "regions": subset, "highlight_cells": hl_cells, 
                        "locked_rows": [], "locked_cols": all_cols.keys()
                    }
...
static func find_r2_hint(board: Array, size: int, regions: Array) -> Dictionary:
    ...
    for r in range(size):
        for c in range(size):
            if board[r][c] == CellState.CAT:
                row_piece[r] = true;col_piece[c] = true;reg_piece[regions[r][c]] = true
    ...
    for reg in range(size):
        ...
        if not reg_piece[reg]:
            for r in range(size):
                for c in range(size):
                    if regions[r][c] == reg and board[r][c] == CellState.EMPTY\
and _can_place(r, c, board, size, regions, row_piece, col_piece, reg_piece):
    ...
    for reg in range(size):
        if reg_piece[reg]: continue
        var cands: Array = reg_cands[reg]
        if cands.size() <= 1: continue
        var rows: Dictionary = {}
        for cell in cands: rows[(cell as Vector2i).x] = true
        if rows.size() == 1:
            var row: int = (cands[0] as Vector2i).x
            var has_new: = false
            for c in range(size):
                if regions[row][c] != reg and board[row][c] == CellState.EMPTY and not row_piece[row]:
                    has_new = true;break
            if has_new:
                return {"found": true, "strategy": "R2", "mode": "r2a_row", 
                    "description": "该色候选都在第%d行，可排除该行其他颜色的格子" % (row + 1), 
                    "region": reg, "row": row, "col": -1, "highlight_cells": cands}
    ...
    for reg in range(size):
        ...
        if cands.size() <= 1: continue
        var cols: Dictionary = {}
        for cell in cands: cols[(cell as Vector2i).y] = true
        if cols.size() == 1:
            var col: int = (cands[0] as Vector2i).y
            var has_new: = false
            for r in range(size):
                if regions[r][col] != reg and board[r][col] == CellState.EMPTY and not col_piece[col]:
                    has_new = true;break
            if has_new:
                return {"found": true, "strategy": "R2", "mode": "r2a_col", 
                    "description": "该色候选都在第%d列，可排除该列其他颜色的格子" % (col + 1), 
                    "region": reg, "row": -1, "col": col, "highlight_cells": cands}
    ...
    for r in range(size):
        ...
        var row_cands: Array = []
        for c in range(size):
            if board[r][c] == CellState.EMPTY and _can_place(r, c, board, size, regions, row_piece, col_piece, reg_piece):
                row_cands.append(Vector2i(r, c))
        if row_cands.size() <= 1: continue
        var row_regs: Dictionary = {}
        for cell in row_cands: row_regs[regions[(cell as Vector2i).x][(cell as Vector2i).y]] = true
        if row_regs.size() == 1:
            ...
            for cell in (reg_cands[reg] as Array):
                if (cell as Vector2i).x != r:
                    has_new = true;break
            if has_new:
                return {"found": true, "strategy": "R2", "mode": "r2b_row", 
                    "description": "第%d行候选均属同一颜色，可排除该颜色其他行的格子" % (r + 1), 
                    "region": reg, "row": r, "col": -1, "highlight_cells": row_cands}


    for c in range(size):
        if col_piece[c]: continue
        var col_cands: Array = []
        for r in range(size):
            if board[r][c] == CellState.EMPTY and _can_place(r, c, board, size, regions, row_piece, col_piece, reg_piece):
                col_cands.append(Vector2i(r, c))
        if col_cands.size() <= 1: continue
        var col_regs: Dictionary = {}
        for cell in col_cands: col_regs[regions[(cell as Vector2i).x][(cell as Vector2i).y]] = true
        if col_regs.size() == 1:
            ...
            for cell in (reg_cands[reg] as Array):
                if (cell as Vector2i).y != c:
                    has_new = true;break
            if has_new:
                return {"found": true, "strategy": "R2", "mode": "r2b_col", 
                    "description": "第%d列候选均属同一颜色，可排除该颜色其他列的格子" % (c + 1), 
                    "region": reg, "row": -1, "col": c, "highlight_cells": col_cands}
...
static func find_chain_hint(board: Array, size: int, regions: Array) -> Dictionary:
    ...
    var best_r: int = -1; var best_c: int = -1

    for r in range(size):
        if (base["placed"] as Array)[r] != -1: continue
        for c in range(size):
            if not (base["cands"] as Array)[r][c]: continue
            var depth: int = _chain_try_contradiction(r, c, size, regions, base)
            if depth >= 0 and depth < best_depth:
                best_depth = depth;best_r = r;best_c = c

    if best_r >= 0:
        var strategy: String = "R4_chain" if best_depth <= 2 else "R5_chain"
        var chain_cells: Array[Vector2i] = [Vector2i(best_r, best_c)]

        var chain_detail: Dictionary = _chain_try_contradiction_detail(best_r, best_c, size, regions, base)
        var desc: String = TranslationServer.translate("HINT_CHAIN_DIRECT") if best_depth == 0\
else TranslationServer.translate("HINT_CHAIN_STEPS") % best_depth
        return {"found": true, "strategy": strategy, 
            "description": desc, 
            "cell": Vector2i(best_r, best_c), "unit_cells": chain_cells, 
            "chain": chain_detail}
    return {"found": false}


static func _chain_try_contradiction_detail(r0: int, c0: int, size: int, regions: Array, base: Dictionary) -> Dictionary:
    var cands: Array = []
    for row_arr in (base["cands"] as Array): cands.append((row_arr as Array).duplicate())
    ...
    while progress:
        ...
        for r in range(size):
            if placed_s[r] != -1: continue
            var cnt: int = 0
            for c in range(size): if cands_s[r][c]: cnt += 1
            if cnt == 0:
                return {"depth": extra, "steps": steps, "contra_type": "row", "contra_index": r}
        for c in range(size):
            if col_s[c]: continue
            var cnt: int = 0
            for r in range(size): if cands_s[r][c]: cnt += 1
            if cnt == 0:
                return {"depth": extra, "steps": steps, "contra_type": "col", "contra_index": c}
        for reg in range(size):
            if reg_s[reg]: continue
            var cnt: int = 0
            for r in range(size):
                for c in range(size): if regions[r][c] == reg and cands_s[r][c]: cnt += 1
            if cnt == 0:
                return {"depth": extra, "steps": steps, "contra_type": "region", "contra_index": reg}

        for r in range(size):
            if placed_s[r] != -1: continue
            var fc: int = -1; var cnt: int = 0
            for c in range(size):
                if cands_s[r][c]:
                    cnt += 1;fc = c
            if cnt == 1:
                ...
                extra += 1;progress = true;break
        if progress: continue
        for c in range(size):
            if col_s[c]: continue
            var fr: int = -1; var cnt: int = 0
            for r in range(size):
                if cands_s[r][c]:
                    cnt += 1;fr = r
            if cnt == 1:
        ...
        for reg in range(size):
            if reg_s[reg]: continue
            var fr2: int = -1; var fc2: int = -1; var cnt: int = 0
            for r in range(size):
                for c in range(size):
...
static func _chain_build_state(board: Array, size: int, regions: Array) -> Dictionary:
    ...
    for _r in range(size):
        var row: Array = [];row.resize(size);row.fill(true);cands.append(row)
    var placed: Array = [];placed.resize(size);placed.fill(-1)
    var col_placed: Array = [];col_placed.resize(size);col_placed.fill(false)
    var reg_placed: Array = [];reg_placed.resize(size);reg_placed.fill(false)
    var state: Dictionary = {"cands": cands, "placed": placed, "col_placed": col_placed, "reg_placed": reg_placed}
    for r in range(size):
        for c in range(size):
            if board[r][c] == CellState.CAT:
...
static func _chain_place(r: int, c: int, size: int, regions: Array, state: Dictionary) -> void :
    ...
    placed[r] = c;col_placed[c] = true;reg_placed[regions[r][c]] = true
    for cc in range(size): if cc != c: cands[r][cc] = false
    for rr in range(size): if rr != r: cands[rr][c] = false
    for dr in range(-1, 2):
        for dc in range(-1, 2):
            if dr == 0 and dc == 0: continue
            var nr: int = r + dr; var nc: int = c + dc
            if nr >= 0 and nr < size and nc >= 0 and nc < size: cands[nr][nc] = false
    var rid: int = regions[r][c]
    for rr in range(size):
        for cc in range(size):
            if regions[rr][cc] == rid and not (rr == r and cc == c): cands[rr][cc] = false


static func _chain_try_contradiction(r0: int, c0: int, size: int, regions: Array, base: Dictionary) -> int:
...
static func compute_r4_plus_cells(board: Array, size: int, regions: Array, solution: Array) -> Dictionary:
