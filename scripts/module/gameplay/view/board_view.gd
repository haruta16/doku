...
const _AUTO_MARK_BTN_SCENE: PackedScene = preload("res://scripts/module/game/ui/compont/auto_mark_btn.tscn")
...
enum ChangeSource{USER_ACTION, RESTORE, LOCATE, HINT, UNDO, AUTOMARK, PREFILL, AUTO_COMPLETE, DRAFT_APPLY}
...
static func resolve_region_palette() -> PackedColorArray:
    ...
    if ABTestManager.region_color.is_custom_palette():
        return PackedColorArray([
            Color("#CBCB24"), Color("#E45F8A"), Color("#8D7AEB"), Color("#F4A2E4"), 
            Color("#FF8E3D"), Color("#F4D27B"), Color("#4B7FC0"), Color("#A2C7ED"), 
            Color("#0AAECF"), Color("#0DA875"), Color("#88CE7A"), Color("#AA7146"), 
        ])
    if ABTestManager.region_color.is_new_cell_only_palette():
        return PackedColorArray([
            Color("#CDA400"), Color("#D36F8F"), Color("#8979DA"), Color("#F89BE5"), 
            Color("#FA9D5C"), Color("#FBD983"), Color("#5076A5"), Color("#A5C6E7"), 
            Color("#38A9C0"), Color("#2A8C53"), Color("#8BD57D"), Color("#A86D4A"), 
        ])
    if ABTestManager.region_color.is_cell_color_v3():
        return PackedColorArray([
            Color("#c9b35b"), Color("#d37291"), Color("#8175bf"), Color("#efa2e0"), 
            Color("#f2a269"), Color("#f0cf7e"), Color("#5580b4"), Color("#a4c5e7"), 
            Color("#4db5ca"), Color("#3fa068"), Color("#a1d388"), Color("#b07959"), 
        ])
    return pal
...
func setup(puzzle_size: int, regions: Array, color_map: Array[int], pattern_regions: Array = [], recycle_reused: bool = false) -> void :
    ...
    if not _ed and ABTestManager.region_color.is_cell_color_v3():
        _color_map = LevelGenerator.compute_color_map_for_rgb(puzzle_size, regions, [
            [201, 179, 91], [211, 114, 145], [129, 117, 191], [239, 162, 224], 
            [242, 162, 105], [240, 207, 126], [85, 128, 180], [164, 197, 231], 
            [77, 181, 202], [63, 160, 104], [161, 211, 136], [176, 121, 89], 
        ])
        ...
        _color_map = LevelGenerator.compute_color_map_for_rgb(puzzle_size, regions, [
            [205, 164, 0], [211, 111, 143], [137, 121, 218], [248, 155, 229], 
            [250, 157, 92], [251, 217, 131], [80, 118, 165], [165, 198, 231], 
            [56, 169, 192], [42, 140, 83], [139, 213, 125], [168, 109, 74], 
        ])
    elif not _ed and ABTestManager.region_color.is_palette_v5():

        _region_colors = PackedColorArray([
            Color("#ac7147"), Color("#f19e80"), Color("#c36a8a"), Color("#afb4d2"), 
            Color("#c58ced"), Color("#a4d987"), Color("#6584b3"), Color("#e4bc4a"), 
            Color("#fea3e9"), Color("#4bb5b1"), Color("#de7e34"), Color("#89c4e6"), 
        ])
        var _rgb_v5: Array = [
            [172, 113, 71], [241, 158, 128], [195, 106, 138], [175, 180, 210], 
            [197, 140, 237], [164, 217, 135], [101, 132, 179], [228, 188, 74], 
            [254, 163, 233], [75, 181, 177], [222, 126, 52], [137, 196, 230], 
        ]
        _color_map = LevelGenerator.compute_color_map_for_rgb_with_pattern(puzzle_size, regions, _rgb_v5, pattern_regions)\
if pattern_regions.size() > 0 else LevelGenerator.compute_color_map_for_rgb(puzzle_size, regions, _rgb_v5)
    elif not _ed and ABTestManager.region_color.is_palette_v6():

        _region_colors = PackedColorArray([
            Color("#c9a779"), Color("#ca6666"), Color("#686dbf"), Color("#9684ec"), 
            Color("#ca7849"), Color("#4aba34"), Color("#9de1e3"), Color("#4cabdb"), 
            Color("#dc5599"), Color("#f4a4e7"), Color("#7ee388"), Color("#dbbc48"), 
        ])
        var _rgb_v6: Array = [
            [201, 167, 121], [202, 102, 102], [104, 109, 191], [150, 132, 236], 
            [202, 120, 73], [74, 186, 52], [157, 225, 227], [76, 171, 219], 
            [220, 85, 153], [244, 164, 231], [126, 227, 136], [219, 188, 72], 
        ]
        _color_map = LevelGenerator.compute_color_map_for_rgb_with_pattern(puzzle_size, regions, _rgb_v6, pattern_regions)\
if pattern_regions.size() > 0 else LevelGenerator.compute_color_map_for_rgb(puzzle_size, regions, _rgb_v6)
    elif not _ed and ABTestManager.region_color.is_palette_v7():

        _region_colors = PackedColorArray([
            Color("#b67c54"), Color("#ffaf92"), Color("#37b95e"), Color("#89c4e4"), 
            Color("#a673d8"), Color("#a4da86"), Color("#6767c3"), Color("#e3bd4d"), 
            Color("#fbafea"), Color("#45b7b3"), Color("#dd8240"), Color("#e4699c"), 
        ])
        var _rgb_v7: Array = [
            [182, 124, 84], [255, 175, 146], [55, 185, 94], [137, 196, 228], 
            [166, 115, 216], [164, 218, 134], [103, 103, 195], [227, 189, 77], 
            [251, 175, 234], [69, 183, 179], [221, 130, 64], [228, 105, 156], 
        ]
        _color_map = LevelGenerator.compute_color_map_for_rgb_with_pattern(puzzle_size, regions, _rgb_v7, pattern_regions)\
if pattern_regions.size() > 0 else LevelGenerator.compute_color_map_for_rgb(puzzle_size, regions, _rgb_v7)
    ...
    for child in get_children():
        if child is CellView and String(child.name).begins_with("Cell_"):
            ...
            if rc.size() == 2 and rc[0].is_valid_int() and rc[1].is_valid_int()\
and int(rc[0]) < puzzle_size and int(rc[1]) < puzzle_size:
    ...
    for r in range(puzzle_size):
        ...
        for c in range(puzzle_size):
            ...
            var ci: int = _color_map[region_idx] if region_idx < _color_map.size() else (region_idx % _region_colors.size())
...
func set_cell_state(r: int, c: int, state: int, is_play_anim: bool = true, show_cat_visual: bool = true, source: int = ChangeSource.USER_ACTION, split_press: bool = false) -> void :
    ...
    (_cells[r][c] as CellView).change_state({"state": state, "play_anim": is_play_anim, "show_cat_visual": show_cat_visual, "split_press": split_press})
    if cur != state:
...
func play_auto_cross(r: int, c: int) -> void :
    ...
    if (_cells[r][c] as CellView).get_state() != CellState.EMPTY:
        return
    (_cells[r][c] as CellView).change_state({"state": CellState.MARK, "play_anim": true, "appear_anim": "AutomaticAppear"})
...
func play_auto_uncross(r: int, c: int) -> void :
    ...
    if cell.get_state() != CellState.MARK:
        return
    cell.change_state({"state": CellState.EMPTY, "play_anim": true, "disappear_anim": "AutomaticDisappear"})
...
func lock_mark(r: int, c: int, lock_anim: String = "", play_anim: bool = true, source: int = ChangeSource.AUTOMARK) -> void :
    if r < 0 or r >= _puzzle_size or c < 0 or c >= _puzzle_size or _cells.is_empty():
        return
    var cell: CellView = _cells[r][c] as CellView
    if cell.get_state() != CellState.MARK:
        return
    cell.change_state({"state": CellState.LOCKED_MARK, "play_anim": play_anim, "lock_anim": lock_anim})
...
func is_region_ever_marked_x(r: int, c: int) -> bool:
    if _regions.is_empty() or _cells.is_empty()\
or r < 0 or r >= _puzzle_size or c < 0 or c >= _puzzle_size:
        return false
    var rid: int = int(_regions[r][c])
    for rr in range(_puzzle_size):
        for cc in range(_puzzle_size):
...
func _rebuild_auto_mark_btns() -> void :
    ...
    for c in range(_puzzle_size):
        ...
        if not _ed:
            var col_idx: int = c
            btn.pressed_with_state.connect( func(is_marked_before: bool) -> void :
