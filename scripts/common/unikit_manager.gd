...
func request_notification_permission(type: int = PUSH_PERMISSION_TYPE_SYSTEM_AND_SETTING, position: String = "default") -> void :
    if _plugin == null:
        push_permission_done.emit.call_deferred()
        return
    if type == PUSH_PERMISSION_TYPE_SYSTEM and OS.has_feature("android") and not _is_android_13_or_above():
        push_permission_done.emit.call_deferred()
        return
...
func add_daily_push(push_id: String, hour_local: float, contents: Array[Dictionary], disturb_type: int = 1) -> void :
    if _plugin == null:
        return
    ...
    var advance_one_day: bool = local["hour"] > target_hour or (local["hour"] == target_hour and local["minute"] >= target_minute)
    ...
    print("[UniKit] add_daily_push: push_id=%s hour=%.1f contents_count=%d disturb=%d" % [push_id, hour_local, contents.size(), disturb_type])
...
func _mock_show_ad(placement_id: String, position: String) -> void :
    ...
    var on_reward_close: = func() -> void :
        ...
        _on_ad_closed(placement_id)
        UIManager.hide_ui(UiName.MOCK_AD)
    UIManager.show_ui(UiName.MOCK_AD, {
...
func _mock_show_banner(placement_id: String, position: String, anchor_bottom: bool, offset_base: int, height_base: int) -> void :
    UIManager.show_ui(UiName.MOCK_BANNER, {
        "placement_id": placement_id, 
        "position": position, 
...
func show_banner(placement_id: String, position: String, anchor_bottom: bool = true, offset_base: int = 0, height_base: int = 180) -> void :
    ...
    for bt in backtraces:
        ...
        for i in range(skip_frames, count):
            lines.append("[%d] %s (%s:%d)" % [
                lines.size(), bt.get_frame_function(i), bt.get_frame_file(i), bt.get_frame_line(i)
            ])
    return "\n".join(lines)
...
func _on_ad_rewarded(placement_id: String) -> void :
    ...
    if _debug_reward_miss and placement_id == "reward":
        print("[UniKit] ad_rewarded SKIPPED by cheat _debug_reward_miss: placement=%s" % placement_id)
        return
...
func _maybe_start_reward_grant_watchdog() -> void :
    ...
    get_tree().create_timer(_REWARD_GRANT_TIMEOUT_SEC).timeout.connect(
        func() -> void : _on_reward_grant_timeout(show_id, position), 
        CONNECT_ONE_SHOT
    )
...
func restore_items_for_position(position: String) -> Array:
    ...
    if position == Tracker.AdPos.PROPS_NORMAL_LOCATE or position == Tracker.AdPos.PROPS_DAILY_LOCATE:
